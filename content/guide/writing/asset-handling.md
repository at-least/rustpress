...
{% <tip kind="tip" title="Linked files are not treated as assets" no_title={false}> %}
...
You can place these files in the `public` directory under the [source directory](/guide/introduction/routing/#source-directory). For example, if your project root is `./docs` and using default source directory location, then your public directory will be `./docs/public`.
...
{% <tip kind="warning" title="CORS Required" no_title={false}> %}
