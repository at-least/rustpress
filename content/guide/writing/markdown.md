...
{% <tip kind="danger" title="" no_title={false}> %}
This is a dangerous warning.
{% </tip> %}
...
{% <tip kind="danger" title="STOP" no_title={false}> %}
Danger zone, do not proceed
{% </tip> %}

{% <details summary="Click me to toggle the code" open={false}> %}
```js
console.log('Hello, VitePress!')
```
{% </details> %}
...
On multilingual sites, these labels can also be overridden per locale - see [Per-locale Markdown Strings](/guide/writing/i18n/#per-locale-markdown-strings).
...
{% <tip kind="note" title="Outer container" no_title={false}> %}
This box contains another container.

{% <details summary="Inner container" open={false}> %}
```js
console.log('Hello, VitePress!')
```
{% </details> %}
...
{% <details summary="Click me to toggle the code" open={true}> %}
```js
console.log('Hello, VitePress!')
```
{% </details> %}
...
```md
::: tip {no-title}
Just want to try it out? Skip to the [Quickstart](/guide/introduction/getting-started/).
:::
```

**Output**

{% <tip kind="tip" title="" no_title={true}> %}
Just want to try it out? Skip to the [Quickstart](/guide/introduction/getting-started/).
{% </tip> %}
