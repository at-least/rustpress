...
```ts,name=docs/.vitepress/config.ts
...
Strings baked into pages by the markdown renderer - the default titles of [custom containers](/guide/writing/markdown/#custom-containers) and [GitHub-flavored alerts](/guide/writing/markdown/#github-flavored-alerts), and the strings of the code copy button - can be overridden per locale with the `markdown` key of a locale entry:
...
```ts,name=docs/.vitepress/theme/index.ts
...
```vue,name=docs/.vitepress/theme/Layout.vue
