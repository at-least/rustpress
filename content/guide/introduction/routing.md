...
Learn more about linking to assets such images in [Asset Handling](/guide/writing/asset-handling/).
...
{% <tip kind="warning" title="Relative Links with Rewrites" no_title={false}> %}
...
The `watch` option works the same way as in [data loaders](/guide/customization/data-loading/#data-from-local-files):
