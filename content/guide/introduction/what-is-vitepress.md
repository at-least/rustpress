...
- **Blogs, Portfolios, and Marketing Sites**
  ...
  VitePress supports [fully customized themes](/guide/customization/custom-theme/), with the developer experience of a standard Vite + Vue application. Being built on Vite also means you can directly leverage Vite plugins from its rich ecosystem. In addition, VitePress provides flexible APIs to [load data](/guide/customization/data-loading/) (local or remote) and [dynamically generate routes](/guide/introduction/routing/#dynamic-routes). You can use it to build almost anything as long as the data can be determined at build time.
...
- **[Built-in Markdown Extensions:](/guide/writing/markdown/)** Frontmatter, tables, syntax highlighting... you name it. Specifically, VitePress provides many advanced features for working with code blocks, making it ideal for highly technical documentation.

- **[Vue-Enhanced Markdown:](/guide/writing/using-vue/)** each Markdown page is also a Vue [Single-File Component](https://vuejs.org/guide/scaling-up/sfc.html), thanks to Vue template's 100% syntax compatibility with HTML. You can embed interactivity in your static content using Vue templating features or imported Vue components.
