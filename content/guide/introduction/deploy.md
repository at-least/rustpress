+++
title = "Deploy Your VitePress Site"
weight = 4
...
- Links created by [`createContentLoader`](/guide/customization/data-loading/#createcontentloader) content stay site-absolute (their HTML is embedded into other pages, so no single relative prefix is correct) — they resolve only for a root mount.
...
{% <details summary="Example Netlify `_headers` file" open={false}> %}
...
Note: the `_headers` file should be placed in the [public directory](/guide/writing/asset-handling/#the-public-directory) - in our case, `docs/public/_headers` - so that it is copied verbatim to the output directory.

[Netlify custom headers documentation](https://docs.netlify.com/routing/headers/)

{% </details> %}

{% <details summary="Example Vercel config in `vercel.json`" open={false}> %}
...
{% raw %}             key: ${{ runner.os }}-vitepress-${{ hashFiles('docs/**', 'package-lock.json', 'pnpm-lock.yaml', 'yarn.lock', 'bun.lockb') }}{% endraw %}
             restore-keys: |
{% raw %}               ${{ runner.os }}-vitepress-{% endraw %}
         - name: Setup Pages
