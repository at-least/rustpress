...
Connecting VitePress to a CMS will largely revolve around [Dynamic Routes](/guide/introduction/routing/#dynamic-routes). Make sure to understand how it works before proceeding.
...
3. Render the content in the page:
    ...
    ```md
{% raw %}    # {{ $params.title }}{% endraw %}

{% raw %}    - by {{ $params.author }} on {{ $params.date }}{% endraw %}
