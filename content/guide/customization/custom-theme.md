...
VitePress will always use the custom theme instead of the default theme when it detects presence of a theme entry file. You can, however, [extend the default theme](/guide/customization/extending-default-theme/) to perform advanced customizations on top of it.
...
```js,name=.vitepress/theme/index.js
...
The `enhanceApp` hook allows you to access the [Vue app instance](https://vuejs.org/api/application.html) and other runtime data, which can be used to [register global components](/guide/customization/extending-default-theme/#registering-global-components), integrate with Vue libraries, etc.
...
```ts,name=.vitepress/theme/index.ts
export default {
  enhanceApp({ router }) {
...
Inside your layout component, it works just like a normal Vite + Vue 3 application. Do note the theme also needs to be [SSR-compatible](/guide/customization/ssr-compat/).

## Building a Layout

The most basic layout component needs to contain a [`<Content />`](/reference/runtime-api/#content) component:

```vue,name=.vitepress/theme/Layout.vue
<template>
  <h1>Custom Layout!</h1>
...
Consult the [Runtime API Reference](/reference/runtime-api/) for everything available in theme components. In addition, you can leverage [Build-Time Data Loading](/guide/customization/data-loading/) to generate data-driven layout - for example, a page that lists all blog posts in the current project.
