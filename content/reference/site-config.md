...
The config file is always resolved from `<root>/.vitepress/config.[ext]`, where `<root>` is your VitePress [project root](/guide/introduction/routing/#root-and-source-directory), and `[ext]` is one of the supported file extensions. TypeScript is supported out of the box. Supported extensions include `.js`, `.ts`, `.mjs`, and `.mts`.
...
{% <details summary="Dynamic (Async) Config" open={false}> %}
...
See [Frontmatter Config](/reference/frontmatter-config/) for details.
...
Alternatively, when using the built-in i18n features, the settings for a locale directory can be overridden via the `locales` setting in the main configuration file. See [Internationalization](/guide/writing/i18n/) for details.
...
- Type: `string`
- Default: `VitePress`
- Can be overridden per page via [frontmatter](/reference/frontmatter-config/#title) or at the [directory level](#directory-level-overrides)
...
- Type: `string | boolean`
- Can be overridden per page via [frontmatter](/reference/frontmatter-config/#titletemplate) or at the [directory level](#directory-level-overrides)
...
- Can be overridden per page via [frontmatter](/reference/frontmatter-config/#description) or at the [directory level](#directory-level-overrides)
...
- Type: `HeadConfig[]`
- Default: `[]`
- Can be appended per page via [frontmatter](/reference/frontmatter-config/#head) or at the [directory level](#directory-level-overrides)
...
Head entries from the site config, [locale config](/guide/writing/i18n/), [directory-level config](#directory-level-overrides), [frontmatter](/reference/frontmatter-config/#head) and [`transformHead`](#transformhead) are merged in that order. A later entry replaces an earlier one with the same key instead of being appended:
...
The one exception is `'./'`, which produces a [relocatable build](/guide/introduction/deploy/#relocatable-builds-relative-base): pages reference everything relative to their own location, so the same output works from any sub path (IPFS gateways, archives) without rebuilding and stays browsable when opened directly from the file system.
...
When set to `true`, VitePress will remove the trailing `.html` from URLs. Also see [Generating Clean URLs](/guide/introduction/routing/#generating-clean-urls).

{% <tip kind="warning" title="Server Support Required" no_title={false}> %}
...
Defines custom directory &lt;-&gt; URL mappings. See [Routing: Route Rewrites](/guide/introduction/routing/#route-rewrites) for more details.
...
The directory where your markdown pages are stored, relative to project root. Also see [Root and Source Directory](/guide/introduction/routing/#root-and-source-directory).
...
The build output location for the site, relative to [project root](/guide/introduction/routing/#root-and-source-directory).
...
The emitted asset URL is `assetsBase` joined with the output-relative file path, so the CDN should mirror the layout of `outDir` (upload `outDir/assets` so it is reachable at `<assetsBase>/assets/*`). HTML pages, Markdown links, [`public`](/guide/writing/asset-handling/#the-public-directory) files and `hashmap.json` stay on [`base`](#base).
...
The directory for cache files, relative to [project root](/guide/introduction/routing/#root-and-source-directory). See also: [cacheDir](https://vite.dev/config/shared-options.html#cachedir).
...
### mpa <span class="VPBadge warning">experimental</span>

- Type: `boolean`
- Default: `false`

When set to `true`, the production app will be built in [MPA Mode](/guide/experimental/mpa-mode/). MPA mode ships 0kb JavaScript by default, at the cost of disabling client-side navigation and requires explicit opt-in for interactivity.
...
When using the default theme, enabling this option will display each page's last updated time. You can customize the text via [`themeConfig.lastUpdated.text`](/reference/default-theme/config/#lastupdated) option.
...
Set `markdown.headers` to `true` or pass [`@mdit-vue/plugin-headers`](https://github.com/mdit-vue/mdit-vue/tree/main/packages/plugin-headers) options to collect headings into [`useData().page.headers`](/reference/runtime-api/#usedata). This option is disabled by default.
...
For simpler cases, it may be possible to use the [`head`](/reference/frontmatter-config/#head) setting in frontmatter, or [`transformPageData`](#transformpagedata).
...
{% <tip kind="note" title="" no_title={false}> %}
