...
You may choose the page layout by setting `layout` option to the page [frontmatter](/reference/frontmatter-config/). There are 3 layout options, `doc`, `page`, and `home`. If nothing is specified, then the page is treated as `doc` page.
...
Option `page` is treated as "blank page". The Markdown will still be parsed, and all of the [Markdown Extensions](/guide/writing/markdown/) work as same as `doc` layout, but it wouldn't get any default stylings.
