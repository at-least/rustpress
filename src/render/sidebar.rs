...
pub fn sidebar<'a>(site: &'a Site, current_url: &str) -> String {
...
fn node<'a>(site: &'a Site, n: &'a SidebarNode, current_url: &'a str, depth: usize) -> String {
    ...
    let section_cls = format!(
        ...
        if starts_collapsed { " collapsed" } else { "" },
    ...
    let aria_expanded = if !starts_collapsed { "true" } else { "false" };
    ...
    rsx! {
        <section class=(section_cls)>
            <div class=(row_cls)>
                ...
                @if collapsible {
                    <button type="button" class={CARET_BTN_CLS} aria-label="toggle section" aria-expanded=(aria_expanded)>
