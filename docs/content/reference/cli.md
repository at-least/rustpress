...
3. writes `syntax.css` (the [syntax color scheme](./site-config#syntax)), `search-docs.json` when [search](./default-theme-search) is on, the selected theme under `themes/` when a [`theme`](../reference/site-config#theme) is set, and `sitemap.xml` when [`[sitemap]`](../guide/sitemap-generation) is configured,
