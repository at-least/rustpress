...
An unknown name or a missing file fails the build. Source-code colors are a separate setting — see [`[syntaxHighlight]`](#syntaxhighlight). Both settings are previewed live in the [Theme Gallery](/themes/) and the [Syntax Highlight Gallery](/themes/syntax-highlight/).
...
See the [Syntax Highlight Gallery](/themes/syntax-highlight/) for live previews of every theme.
