---
title: Theming
description: Change the colors — a UI theme and a syntax theme, two one-liners in rustpress.toml. Deeper customization is not the goal; VitePress is the tool for that.
---
...
One line per color mode. Every bundled theme is previewed live in the [Syntax Theme Gallery](/themes/syntax/); the name in the card is the value. The exact behavior is in the [`[syntax]` reference](../reference/site-config#syntax).
